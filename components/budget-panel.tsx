...
function BudgetPanelInner({ tripId }: { tripId: string }) {
  ...
  // Bars are sized relative to the largest category so the leading spend fills
  // the rail; this keeps the proportions legible whatever the trip total is.
  const maxCategory = categories.reduce((max, cat) => Math.max(max, cat.total), 0);
        ...
        <dl className="flex w-full max-w-xs flex-col gap-2">
          {categories.map((cat) => (
            <div key={cat.label} className="flex flex-col gap-1">
              ...
              <div
                className="wayfarer-budget-bar-track"
                role="presentation"
                aria-hidden
              >
                <div
                  className="wayfarer-budget-bar-fill"
                  style={{
                    width: `${
